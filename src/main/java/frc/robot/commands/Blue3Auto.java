...
/**
 * Blue3 / Red3 自動路線（同一份程式，自動依聯盟鏡像）
 * 包含兩段路徑與兩次定點射擊。
 */
public class Blue3Auto extends SequentialCommandGroup {
    ...
    public Blue3Auto(
        ...
        try {
            // 嘗試載入自動路線檔案
            path1 = PathPlannerPath.fromPathFile("Blue3-1");
            path2 = PathPlannerPath.fromPathFile("Blue3-2");
        } catch (Exception e) {
            e.printStackTrace();
            addCommands(Commands.print("[Blue3Auto] 路徑載入失敗，自動階段中止！"));
            return;
        }
        ...
        addCommands(
            ...
            // 1. 跑 Blue3-1，並在出發後延遲指定時間才伸出進件機構 (Intake) 收球
            new ParallelDeadlineGroup(
                AutoBuilder.followPath(path1),
                new SequentialCommandGroup(
                    new WaitCommand(1.3), // ⬅️ 修改這裡的數字：決定出發後幾秒要降下進件機構
            ...
            // 若你需要發射後等待更久，可以修改這裡的 wait 時間
            new WaitCommand(4.0),

            // 3. 跑 Blue3-2，並在出發後延遲指定時間才啟動進件機構 (Intake)
            new ParallelDeadlineGroup(
                AutoBuilder.followPath(path2),
                new SequentialCommandGroup(
                    new WaitCommand(2), // ⬅️ 修改這裡的數字：決定跑第二段時幾秒後降下進件機構
                    new InstantCommand(intake::extend, intake),
                    new InstantCommand(roller::runRollers, roller)
                )
            ),
